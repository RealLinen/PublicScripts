...
local function format_output(keys) -- i used to use a proper table formatter but this is faster and shorter
    local encoded = http_service:JSONEncode(keys);
    local indent = (" "):rep(4); -- why is \t so damn long
    
    local formatted = ("Date : %s\n\n%s"):format(os.date(), encoded:gsub(",", ",\n"):gsub("{", "{\n"):gsub("}", "\n}"):gsub(":", " : "));

    for index in formatted:gmatch('"%a+" :') do 
        formatted = formatted:gsub(index, indent .. index);
    end;
